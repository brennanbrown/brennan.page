...
### Basic Command Execution (ALWAYS USE THIS)
```bash
...
### Using Here-Document for Complex Scripts
```bash
...
# Using rsync (preferred for directories, excludes .git)
...
```bash
# 1. Create file locally with desired content
...
# For complete file replacement (small files only)
...
# Install updates (with -y for non-interactive)
ssh -i ~/.omg-lol-keys/id_ed25519 -T -o BatchMode=yes root@159.203.44.169 "
  apt upgrade -y
"
```
...
# Add firewall rule (ALWAYS ensure port 22 is open first!)
ssh -i ~/.omg-lol-keys/id_ed25519 -T -o BatchMode=yes root@159.203.44.169 "
  ufw allow 22/tcp comment 'SSH Primary'
  ufw allow 2222/tcp comment 'SSH Backup'
  ufw allow 80/tcp comment 'HTTP'
  ufw allow 443/tcp comment 'HTTPS'
"

# Enable UFW (DANGEROUS - only after ports are allowed!)
ssh -i ~/.omg-lol-keys/id_ed25519 -T -o BatchMode=yes root@159.203.44.169 "
  echo 'y' | ufw enable
"
```
...
### Verbose SSH for Debugging Connection Issues
```bash
# Add -v for verbose output (helps debug hanging issues)
ssh -v -i ~/.omg-lol-keys/id_ed25519 -T -o BatchMode=yes root@159.203.44.169 "command"

# Use -vv or -vvv for even more verbose output
ssh -vvv -i ~/.omg-lol-keys/id_ed25519 -T -o BatchMode=yes root@159.203.44.169 "command"
```
...
```bash
# Run command in background (for long operations)
ssh -i ~/.omg-lol-keys/id_ed25519 -T -o BatchMode=yes root@159.203.44.169 "
...
# If using non-root user, configure passwordless sudo first:
ssh -i ~/.omg-lol-keys/id_ed25519 -T -o BatchMode=yes root@159.203.44.169 "
...
**Solution**: Use the download → edit → upload workflow (see File Editing section above)
...
```bash
# 1. Create/modify files locally in ~/homelab/
...
2. **ALWAYS** add both port 22 and 2222 before enabling UFW
3. **ALWAYS** use `-T` and `-o BatchMode=yes` for non-interactive operations
4. **NEVER** commit SSH keys or passwords to git
5. **ALWAYS** test firewall rules in a safe order
6. **ALWAYS** keep local backups before making server changes
